...
public class Mochila extends JFrame implements ActionListener {
	...
	void obtenerPrecio(){
		if     (marca==0){pre=149.9;}
		else if(marca==1){pre=89.9;}
		else if(marca==2){pre=119.9;}
		else 			 {pre=174.9;}
	}
	void ImporteCompra(){
		impC=pre*cant;
	...
	void ImporteDescuento(){
		...
		if     (cant < 5){impD=0;}
		else if(cant >=5 && cant < 7){impD=0.05*impC;}
		else if(cant >=7 && cant < 9){impD=0.07*impC;}
		else 						 {impD=0.09*impC;}
	}
	void ImportePagar(){
		impP=impC-impD;
	...
	void obsequio(){
		if 		(impP<200){obs=0;}
		else if (impP>=200 && impP<500){obs=1*cant;}
		else if (impP>=500 && impP<700){obs=2*cant;}
		else						   {obs=3*cant;}
	}
